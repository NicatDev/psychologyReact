...
export default function TestList({ user }: any) {
  ...
  return (
    <div className="container mx-auto mt-10 mb-11 bg-white p-8 rounded shadow">
      <h1 className="text-3xl font-semibold mb-6">{t("profile.test_results")}</h1>
      {tests?.length ?
                ...
                >
                  <div>
                    <h2 className="text-xl font-semibold flex items-center gap-2">
                      ...
                      {t("profile.test_title_prefix")}
                    </h2>
                  </div>
                  <span className="text-blue-600">
                    {openTestId === test.id ? t("profile.collapse") : t("profile.view")}
                  </span>
                </div>
                      ...
                      <div className="space-y-4">
                              ...
                              <div className="flex items-center gap-4 justify-center">
                                {question.options.map((opt) => (
                                  ...
                                  >
                                    ...
                                    />
                                    <div
                                      className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${testData.answers.includes(opt.id)
                                          ? "border-blue-500"
                                          : "border-gray-300"
                                        }`}
                                    >
                        ...
                        <div className="flex gap-2 justify-center mt-4">
                          <button
                            ...
                            className={`px-4 py-2 rounded ${isFirstPage ? "bg-gray-300 cursor-pointer" : "bg-blue-500 text-white"
                              }`}
                          >
                            {t("blogs.prev")}
                          </button>
                          <button
                            ...
                            className={`px-4 py-2 rounded ${isLastPage ? "bg-gray-300 cursor-pointer" : "bg-blue-500 text-white"
                              }`}
                          >
                            {t("blogs.next")}
                          </button>
      ...
      {!tests?.length ? <div className="space-y-4 caret-gray-400">{t("profile.no_history")}</div> : <></>}
    </div>
  );
}
