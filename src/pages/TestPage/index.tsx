...
const TestPage = () => {
  ...
  const nextQuestions = () => {
    ...
    setSetCompleted([
      ...setCompleted.slice(0, currentQuestionSet),
      true,
      ...setCompleted.slice(currentQuestionSet + 1),
    ]);
    window.scrollTo(0, 0);
  };
  ...
  const questions = [
    ...
    "Yeni vəziyyətlər və məsələlər qarşısında necə davranırsınız?",
    "Həyatınızda daim məqsədləriniz və planlarınız var?",
    "Dostlarınızla vaxt keçirməyi necə qiymətləndirirsiniz?",
    "Yeni iş və təcrübələrə açıq mısınız?",
  ...
  const trueOptions = [
    ...
    "Sıx əlaqədə oluram",
    "Özüm həll edirəm",
    "Sevə-Sevə qarşılayıram",
    "Bəli",
    "Çox vacibdir",
    "Bəli",
    "Onun fikirlərini dinləmək",
  ];
  const falseOptions = [
    " Xeyr",
    "Yalnız olmaq",
    "Geri planda qalıram",
    "Fikirlərimi qoruyuram",
    "Başqalarının köməyini axtarıram",
    "Qaçıram",
    " Xeyr",
    "Yalnız olmağı tərəccüb edirəm",
    " Xeyr",
    "Öz fikirlərinizi bölüşmək",
  ];
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      renderedQuestions.push(
        <div
          ...
          key={i}
        >
          <p className="test-quiz pt-6 text-[14px] md:text-[20px]">{questions[i % 10]}</p>
          <div className="input flex justify-center gap-14">
            <p className="text-[14px] md:text-[20px]" style={{ width: "30%", fontWeight: 500, color: "#404f69" }}>
              {trueLabel}
            </p>
            <div className="input-flex flex gap-1 md:gap-1 justify-center">
              {levelOptions.map((option, j) => (
                <label key={j} className="cursor-pointer container !p-0">
                  <input
                    type="radio"
                    value={option}
                  ...
                  />
                  <div className="bg-white w-4 h-4 md:w-8 md:h-8 p-1 md:p-2 duration-300 border-primary-blue border border-solid rounded-full peer-checked:bg-primary-blue flex items-center justify-center">
            ...
            <p className="text-[14px] md:text-[20px]" style={{ width: "30%", fontWeight: 600, color: "#404f69" }}>
              {falseLabel}
            </p>
          </div>
        </div>
  ...
  return (
    <div className="bg-zinc-100">
      <div className="container mx-auto px-2 py-20">
        <h2 className="text-center mb-4 font-bold text-sky-950 text-2xl">
          SON 30 GÜNDƏ 973.578 TEST KEÇİRİLİB
        </h2>
        <p className="style-p mb-8 text-lg text-center">
          Bu pulsuz şəxsiyyət testi sizə 9 şəxsiyyət tipindən hansının sizə ən
          uyğun olduğunu göstərəcək. Bütün 9 Şəxsiyyət tipi üçün necə xal
          qazandığınıza baxın və şəxsiyyət sistemində harada yerləşdiyinizi
          anlayın.
        </p>
        <div>
          <h3 className="bg-primary-blue text-white p-4 text-center font-medium">
            ŞƏXSİYYƏT TESTİNDƏN KEÇMƏK ÜÇÜN HƏR BƏYANATI SİZİN ŞƏXSİYYƏTİNİZİ
            NECƏ DƏRƏCƏ TƏSVİR EDİLMƏSİ ƏSASINDA QEYD EDİN.
          </h3>
