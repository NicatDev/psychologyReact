...
const TestResult = () => {
  ...
  useEffect(() => {
    if (userType && (personalityTypes as any)[userType]) {
      setResult((personalityTypes as any)[userType]);
    }
  }, [userType]);
  ...
  return (
    <>
      <Helmet>
        <title>{userType} - Şəxsiyyət Testi Nəticəsi | Octopus</title>
        <meta
          name="description"
          content={`Bu səhifədə ${userType} tip şəxsiyyətinin nəticələri, güclü tərəfləri, zəif tərəfləri və tövsiyələri göstərilir.`}
        />
      ...
      <div className="">
        ...
        <div className="container mx-auto px-2 py-20 flex justify-between gap-10 flex-col md:flex-row">
          <div className="w-full md:w-full flex flex-col gap-6 text-lg">
            ...
            {renderList(
              "Təşkilata töhfələr",
              result.contributionsToOrganization
            )}
            <p className="mt-8 font-semibold text-lg">
              Komanda ilə işləmə tərzi
            </p>
            ...
            {renderList(
              "Başkalarının stressdə köməyi",
              result.othersHelpStress
            )}
            {renderList(
              "Ən pis stress reaksiyaları",
              result.worstStressResponse
            )}
            {renderList(
          ...
          <div className="border border-solid border-zinc-300 w-full md:w-2/5 rounded-lg h-fit overflow-hidden ">
            ...
            <div className="p-4">
              <SidebarChart
                scores={
                  user?.tests?.find(
                    (test: any) => test.id === Number(testSelected)
                  )?.result_values || {}
                }
                scaleData={scaleData}
              />
